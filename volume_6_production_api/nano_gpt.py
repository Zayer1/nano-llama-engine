...
vocab_size = 65 # Tiny Shakespeare has exactly 65 unique characters
embedding_dim = 64
num_heads = 4
...
class NanoGPT(nn.Module):
    ...
    def generate(self, idx, max_new_tokens):
        ...
        for _ in range(max_new_tokens):
            ...
            # Get the predictions
            logits, _ = self(idx_cond) #Returns (Batch, Time, Vocab_Size)
            
            # Focus only on the last time step
            logits = logits[:, -1, :] # becomes (Batch, Vocab_Size)
            #We care only about what comes out at the end of the sentence, hence T[-1] here
